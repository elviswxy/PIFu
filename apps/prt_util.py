...
def own_testPRT(dir_path, n=40):
    if dir_path[-1] == '/':
        dir_path = dir_path[:-1]
    sub_name = dir_path.split('/')[-2][:-5]
    print(sub_name)
    obj_path = os.path.join(dir_path, sub_name + '_30k.obj')
    PRT, F = computePRT(obj_path, n, 2)
    output_path = '/hd4/renderpeople/models/prt_data/{}'.format(sub_name)
    os.makedirs(os.path.join(output_path, 'bounce'), exist_ok=True)

    # PRT, F = computePRT(obj_path, n, 2)
    np.savetxt(os.path.join(output_path, 'bounce', 'bounce0.txt'), PRT, fmt='%.8f')
    np.save(os.path.join(output_path, 'bounce', 'face.npy'), F)
